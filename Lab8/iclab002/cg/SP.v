...
//clk gating
//control signal = 1 indicates that clk is gated
wire G_clock_in;
reg control_in;
wire G_clock_data_reg;
reg control_data_reg;
wire G_clock_out;
reg control_out;
wire G_clock_max_min;
reg control_max_min;
wire G_clock_sma;
reg control_sma;
wire G_clock_add_sub;
reg control_add_sub;
// wire G_clock_median;
// reg control_median;
// wire G_clock_extra;
...
// Clock Gating
//========================================================
GATED_OR GATED_in(.CLOCK(clk), .SLEEP_CTRL(control_in && cg_en), .RST_N(rst_n), .CLOCK_GATED(G_clock_in));
GATED_OR GATED_data_reg(.CLOCK(clk), .SLEEP_CTRL(control_data_reg && cg_en), .RST_N(rst_n), .CLOCK_GATED(G_clock_data_reg));
GATED_OR GATED_out(.CLOCK(clk), .SLEEP_CTRL(control_out && cg_en), .RST_N(rst_n), .CLOCK_GATED(G_clock_out));
GATED_OR GATED_max_min(.CLOCK(clk), .SLEEP_CTRL(control_max_min && cg_en), .RST_N(rst_n), .CLOCK_GATED(G_clock_max_min));
GATED_OR GATED_sma(.CLOCK(clk), .SLEEP_CTRL(control_sma && cg_en), .RST_N(rst_n), .CLOCK_GATED(G_clock_sma));
GATED_OR GATED_add_sub(.CLOCK(clk), .SLEEP_CTRL(control_add_sub && cg_en), .RST_N(rst_n), .CLOCK_GATED(G_clock_add_sub));
//GATED_OR GATED_median(.CLOCK(clk), .SLEEP_CTRL(control_median && cg_en), .RST_N(rst_n), .CLOCK_GATED(G_clock_median));
//GATED_OR GATED_extra(.CLOCK(clk), .SLEEP_CTRL(control_extra && cg_en), .RST_N(rst_n), .CLOCK_GATED(G_clock_extra));

//control signal
always@ (*) begin
	control_in = (current_state == IDLE) ? 1'b0 : 1'b1;
end

always@ (*) begin
	if(current_state == OUTPUT) begin
		control_data_reg = 1'b1;
	end
	else if(in_mode_reg[2] && current_state == SMA) begin
		control_data_reg = 1'b1;
	end
	else begin
		control_data_reg = 1'b0;
	end
end

always@ (*) begin
	control_out = (current_state != OUTPUT) && !out_valid;
end

always@ (*) begin
	//control_max_min = (current_state != IDLE) && (current_state != INPUT);
	if(current_state == IDLE) begin
		control_max_min = 1'b0;
	end
	else if(current_state != IDLE && current_state != INPUT) begin
		control_max_min = 1'b1;
	end
	else if(!in_mode_temp[1]) begin
		control_max_min = 1'b1;
	end
	else begin
		control_max_min = 1'b0;
...
always@ (*) begin
	if(!in_mode_reg[1]) begin
		control_add_sub = 1'b1;
	end
	else if(in_mode_reg[1] && current_state != INPUT) begin
		control_add_sub = 1'b1;
	end
	else begin
		control_add_sub = 1'b0;
	end
end

always@ (*) begin
	if(!in_mode_reg[2]) begin
		control_sma = 1'b1;
	end
	else if(in_mode_reg[2] && current_state != SMA) begin
		control_sma = 1'b1;
...
// always@ (*) begin
// 	control_median = (current_state != OUTPUT);
// end

// assign control_extra = 1'b1;
...
//in_data_reg
always@ (posedge G_clock_data_reg or negedge rst_n) begin
	if(!rst_n) begin
...
//add_sub_reg
always@ (posedge G_clock_add_sub or negedge rst_n) begin
	if(!rst_n) begin
		for(i = 0; i < 9; i = i + 1) begin
			add_sub_reg[i] <= 0;
		end
...
//sma_reg
always@ (posedge G_clock_sma or negedge rst_n) begin
	if(!rst_n) begin
		for(i = 0; i < 9; i = i + 1) begin
			sma_reg[i] <= 0;
		end
...
//find max
always@ (posedge G_clock_max_min or negedge rst_n) begin
	if(!rst_n) begin
		max <= -256;
	end
	else begin
...
// always@ (posedge G_clock_extra or negedge rst_n) begin
// 	if(!rst_n) begin
// 		extra_count <= {40{2'b01}};
// 	end
// 	else begin
